...
def JonesKnotGeoTwist(num,denom):
    ...
    for i in range(len(loops)):
        ...
        if loops[i] == 'L':
            ...
            if path[i+1] > path[i]:
                currPow -= 2
                if (path[i+1] - u) <= -(-v // 2) and sign == 1 or (path[i+1] - u) > -(-v // 2) and sign == 0:
                    sign = sign - 1
            else:
                currPow += 2
                if path[i+1] <= -(-u // 2) and sign == 1 or path[i+1] > -(-u // 2) and sign == 0:
                    sign = sign - 1
...
print(JonesKnotGeoTwist(17, 4). as_expr())
print(geoJonesKnot(17,4).as_expr())
print(AlgJonesKnot(17,4).as_expr())
