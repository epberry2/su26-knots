...
def colored_homfly_geo(num, denom, j):
    ...
    for key, (m, n) in values.items():
        idx = 0
        a_pow, q_pow = m - min_m, n - min_n
        parity = prod(parities[x] for x in key)
        ...
        left_key = key[:idx]
        right_key = key[idx:]
        for left in set(permutations(left_key)):
            basis_el = idx
            for right in set(permutations(right_key)):
               _,  offset = sorted_and_offset(left + right)
               sol[-1 -basis_el] += parity * a**a_pow * q**(q_pow + offset)
    return sol
